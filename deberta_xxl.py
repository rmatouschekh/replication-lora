...
from transformers import AutoTokenizer, Trainer, TrainingArguments, DebertaV2ForSequenceClassification, set_seed
...
def make_lora_layer(layer, lora_r=8): # Set lora_r according to Table 10
    new_layer = lora.Linear(
        in_features=layer.in_features,
        out_features=layer.out_features,
        bias=layer.bias is None,
        r=lora_r,
        lora_alpha=8, # Set lora_alpha according to Table 10
        merge_weights=False
    )
...
tokenizer = AutoTokenizer.from_pretrained(model_name, model_max_length=64) # UPDATE: model_max_length -- LoRA paper: Table 10
model = DebertaV2ForSequenceClassification.from_pretrained(model_name, num_labels=2).to(device=device) # UPDATE: num_labels -- GLUE task

# Mark only the query and value parameters as trainable
for i in range(len(model.deberta.encoder.layer)):
  model.deberta.encoder.layer[i].attention.self.query_proj = make_lora_layer(model.deberta.encoder.layer[i].attention.self.query_proj)
  model.deberta.encoder.layer[i].attention.self.value_proj = make_lora_layer(model.deberta.encoder.layer[i].attention.self.value_proj)
...
training_args = TrainingArguments(
    output_dir="./results",
    num_train_epochs=10, # UPDATE -- Table 10
    per_device_train_batch_size=4, # UPDATE -- Table 10
    per_device_eval_batch_size=4, # UPDATE -- Table 10
    evaluation_strategy="epoch",
    learning_rate=1e-4, # UPDATE -- Table 10
    save_strategy="epoch",
    load_best_model_at_end=True,
    save_total_limit=2,
    warmup_ratio=0.1, # Set according to Table 10
    weight_decay=0 # UPDATE -- Table 10
)
...
trainer.train()
trainer.save_model("models/deberta-xxl-cola-109") # UPDATE -- GLUE task and experiment iteration
trainer.evaluate()
...
predictions_df.to_csv("outputs/deberta_xxl/109/CoLA.tsv", sep="\t") # UPDATE -- GLUE task