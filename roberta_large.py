...
def make_lora_layer(layer, lora_r=8): # Set lora_r according to Table 9
    new_layer = lora.Linear(
        ...
        r=lora_r,
        lora_alpha=16, # Set lora_alpha according to Table 9
        merge_weights=False
    )
...
model = RobertaForSequenceClassification.from_pretrained(model_name, num_labels=2).to(device=device) # UPDATE: num_labels -- GLUE task
...
training_args = TrainingArguments(
    output_dir="./results",
    num_train_epochs=10, # UPDATE -- Table 9
    per_device_train_batch_size=4, # UPDATE -- Table 9
    per_device_eval_batch_size=4, # UPDATE -- Table 9
    evaluation_strategy="epoch",
    learning_rate=2e-4, # UPDATE -- Table 9
    save_strategy="epoch",
...
trainer.train()
trainer.save_model("models/roberta-large-cola-109") # UPDATE -- GLUE task and experiment iteration
trainer.evaluate()
...
predictions_df.to_csv("outputs/roberta_large/109/QNLI.tsv", sep="\t") # UPDATE -- GLUE task